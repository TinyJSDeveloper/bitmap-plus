...
impl Bitmap {
    ...
    // ocorra alguma falha.
    pub fn from_file(content: Vec<u8>) -> Result<Bitmap, Box<Error>> {
        ...
        if content_length < 0x0E {
            return Err("Content length is too short to even contain a header (minimum 14 bytes).".into());
        }
        ...
        if  header_signature != "BM" {
            return Err("Content is not a bitmap file (they start with \"BM\").".into());
        }
        ...
        let header_file_size: u32 = u32::from_le_bytes([content[0x02], content[0x03], content[0x04], content[0x05]]);
        ...
        if header_file_size != content_length {
            return Err("Content length doesn't match with header.".into());
        }

        // Metadados da imagem. Estes são na verdade 2 valores de 16 bits, mas
        // seu uso é reservado e varia entre as aplicações.
        let header_metadata: u32 = u32::from_be_bytes([content[0x06], content[0x07], content[0x08], content[0x09]]);

        // Offset de onde os dados da imagem começam.
        let header_offset: u32 = u32::from_le_bytes([content[0x0A], content[0x0B], content[0x0C], content[0x0D]]);
        ...
        // 256 cores.
        //
        // Curiosamente, todos os bitmaps com 256 cores indexadas começam, no
        ...
        if header_offset != 0x436 {
            return Err("Only 256-color bitmaps are supported.".into());
        }

        // Dimensões de largura e altura da imagem.
        let dib_bitmap_width : u32 = u32::from_le_bytes([content[0x12], content[0x13], content[0x14], content[0x15]]);
        let dib_bitmap_height: u32 = u32::from_le_bytes([content[0x16], content[0x17], content[0x18], content[0x19]]);
        ...
        if dib_bitmap_width % 2 == 1 || dib_bitmap_height % 2 == 1 {
            return Err("Only power of 2 bitmaps are supported.".into());
        }

        // Número de bits por pixel (256 cores equivale a 8bpp).
        let dib_color_bpp: u16 = u16::from_le_bytes([
            content[0x1C], content[0x1D]
        ]);
        ...
        if dib_color_bpp != 8 {
            return Err("Only 8bpp bitmaps are supported.".into());
        }

        // Método de compressão usado para o bitmap.
        let dib_compression: u32 = u32::from_le_bytes([
            content[0x1E], content[0x1F], content[0x20], content[0x21]
        ]);
        ...
        if dib_compression != 0 {
            return Err("Only uncompressed bitmaps are supported.".into());
        }
        ...
        let data: Vec<u8> = content[
            (header_offset as usize..content_length as usize)
        ].to_vec();

        // Percorrer conteúdo em torno da paleta de cores...
        for i in 0..256 {
            palette.push(Color::new(
                content[(0x36 + (i * 4) + 2) as usize],
                content[(0x36 + (i * 4) + 1) as usize],
                content[(0x36 + (i * 4)    ) as usize],
                content[(0x36 + (i * 4) + 3) as usize]
            ));
        }
    ...
    pub fn export_file(&mut self) -> Result<Vec<u8>, Box<Error>> {
        // @error
        //
        ...
        if self.width < 1 || self.height < 1 {
            return Err("Bitmap size (width and height) must be at least greater than zero.".into());
        }
        ...
        if self.data.len() < 1 {
            return Err("Bitmap image data must have at least 1 pixel.".into());
        }
        ...
        let mut content: Vec<u8> = vec![
            ...
            // Tamanho total do conteúdo, em bytes.
            file_size_bytes[0], file_size_bytes[1], file_size_bytes[2], file_size_bytes[3],
            ...
            // Largura da imagem.
            width_bytes[0], width_bytes[1], width_bytes[2], width_bytes[3],
        ...
        let default_color: Color = Color::new(0, 0, 0, 0);

        // @template [u8[256 * 4]; little-endian]
        // Paleta de cores da imagem.
        for i in 0..256 {
            // Obter referência da array padrão:
            let mut color: &Color = &default_color;
    ...
    pub fn export_framebuffer(&self) -> Vec<u32> {
        ...
        for i in 0..self.data.len() {
            ...
            let color: &Color = self.palette.get_color(pixel as usize);
            content.push(color.to_hex());
        }
    ...
    pub fn is_onscreen(&self, x: i32, y: i32, width: i32, height: i32) -> bool {
        ...
        return (left  >= 0 && left  < self.width && top    >= 0 && top    < self.height) ||
    ...
    ///
    /// Um valor booleano que indica êxito (`true`) ou falha (`false`).
    pub fn draw_image_part(&mut self, x: i32, y: i32, image: &Bitmap, mask: Option<u8>, blit: Option<u8>, mut x_scale: i32, mut y_scale: i32, x_start: i32, y_start: i32, width: i32, height: i32) -> bool {
        ...
        for i in 0..height * y_scale as i32 {
            for j in 0..width * x_scale as i32 {
                // Pixel do sprite:
                let mut pixel: u8 = image.get_pixel((j + (x_start * x_scale)) / x_scale, (i + (y_start * y_scale)) / y_scale);
                ...
                if pixel != alpha {
                    ...
                    if blit.is_some() {
                        if self.get_pixel(x + j + x_start, y + i + y_start) == blit.unwrap() {
    ...
    ///
    /// Um valor booleano que indica êxito (`true`) ou falha (`false`).
    pub fn draw_image(&mut self, x: i32, y: i32, image: &Bitmap, mask: Option<u8>, blit: Option<u8>, x_scale: i32, y_scale: i32) -> bool {
        return self.draw_image_part(x, y, image, mask, blit, x_scale, y_scale, 0, 0, image.get_width(), image.get_height());
    }

    /// @todo COmentar e testar depois.
    pub fn draw_frame(&mut self, x: i32, y: i32, image: &Bitmap, mask: Option<u8>, blit: Option<u8>, x_scale: i32, y_scale: i32, frame: &Frame) -> bool {
        let x_start: i32 = frame.get_x_start();
        let y_start: i32 = frame.get_y_start();
        let width  : i32 = frame.get_width();
        let height : i32 = frame.get_height();

        return self.draw_image_part(x, y, image, mask, blit, x_scale, y_scale, x_start, y_start, width, height);
    }
